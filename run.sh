...
sudo docker run -d --rm --net host -e ACCESS_KEY="${ACCESS_KEY}" -e SECRET_ACCESS_KEY="${SECRET_ACCESS_KEY}" \
  -e FOLDER_NAME="${FOLDER_NAME}" -e BUCKET_NAME="${BUCKET_NAME}" -e STORAGE_PROVIDER="${STORAGE_PROVIDER}" --log-driver json-file --log-opt max-size=10m --log-opt max-file=5 seekret/sniffer:2
